...
public class GenerarSetDTEController {
	...
	/**
	 * Genera un set de DTE jaxb a partir de los datos de dte almacenados en la base
	 * de datos
	 * 
	 * @param payload request para generar el set
	 * @return objeto {@link JSend} simple
	 */
	@RequestMapping(method = RequestMethod.POST, value = "/generarsetdte", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<JSend> generarDtesSetDTE(@RequestBody GenerarSetDTERequest payload) {
		try {
			...
			log.debug(payload.getTipoDocumento().toString());
			...
			log.debug("Tipos de dto en set: " + payload.getTipoDocumento().toString());

			ConvertRequestToEnvioDTEResponse response = mapperToDTESetDTE.toEnvioDTESetDTE(emisor.get(), rutfirmante,
					payload, payload.getTipoDocumento());
			if (response == null) {
			...
			if (response.getDteList().isEmpty()) {
			...
			Setdte newsetdte = generateEntityFromJaxbEnvioDTESetDTE(response.getXmldtes(), documento, rutemisor,
					rutfirmante, response.getDteList(), payload.getTipoDocumento());
			...
			return ResponseEntity.ok()
					.body(JSend.success("set de DTE generado con id=" + newsetdte.getIdSetdte()));
	 ...
	 */
	@Transactional
	public Setdte generateEntityFromJaxbEnvioDTESetDTE(String xmldtes, EnvioDTE.SetDTE documento, String rutemisor,
			String rutfirmante, List<Long> dteList, Integer tiposDocumentoEnSet) {
		try {
			if (rutemisor == null) {
			...
			envDTE.setSetDTE(documento); // <--aqui se traspasa la lista de dtesY
			...
			Integer tipoDte = tiposDocumentoEnSet;
			switch(tipoDte) {
				case 33:
					newSetDte.setTipoSet(EntitySetDteTipoSet.FACTURA_AFECTA.getValue());
					break;
				case 34:
					newSetDte.setTipoSet(EntitySetDteTipoSet.FACTURA_EXENTA.getValue());
					break;
				case 61:
					newSetDte.setTipoSet(EntitySetDteTipoSet.NOTA_CREDITO.getValue());
					break;
				case 56:
					newSetDte.setTipoSet(EntitySetDteTipoSet.NOTA_DEBITO.getValue());
					break;
				case 52:
					newSetDte.setTipoSet(EntitySetDteTipoSet.GUIA_DESPACHO.getValue());
					break;
				case 39:
					newSetDte.setTipoSet(EntitySetDteTipoSet.BOLETA.getValue());
					break;
				case 41:
					newSetDte.setTipoSet(EntitySetDteTipoSet.BOLETA_EXENTA.getValue());
					break;
				default:
					newSetDte.setTipoSet(EntitySetDteTipoSet.DTE.getValue());
					break;
			}
			newSetDte.setTipoSet(EntitySetDteTipoSet.DTE.getValue());
			...
			xmlcontent = xmlcontent.replaceAll(startString + "[\\s\\S]*" + endString,
					startString + xmldtes + SiiDocumentFactoryConfiguration.CARRIAGE_RETURN + endString);
			...
			if (rutfirmante == null) {
				...
				if (kinfo == null) {
					...
					if (xmlsetdteSigned == null) {
						log.error("Ocurrio un error firmando URI=" + setdteURI + ". No fue posible firmar SetDTE ID="
								+ ID);
						throw new Exception("No fue posible generar DTE ID=" + ID);
					}

					xmlsetdteSigned = xmlsetdteSigned.replace("<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>",
			...
			dteList.forEach(dteid -> {
				...
				bitacora.setProceso("generateEntityFromJaxbEnvioDTESetDTE");
