    ...
    public List<DTEAndVerbatimCAF> toDTEDocumento(GenerarDTERequest request, Integer tipoDocumento) {
        ...
        for (DTEDocumento documento : request.getDtes()) {
            ...
            try {
                ...
                JAXBElement<DTE.Documento.TED.DD> jaxbElement = new JAXBElement<>(
                    new QName("DD"), DTE.Documento.TED.DD.class, dd);
                marshallerDD.marshal(jaxbElement, mywriter);
                mywriter.flush();
    ...
    private DTE.Documento.Encabezado.Totales obtenerTotales(DTEDocumento documento, Double tasaIva) {
        try {
            ...
            for (DTEDetalle detalle : documento.getDetalle()) {
                ...
                log.debug("Monto: " + detalle.getMontoitem().toString());
            ...
            log.debug("Indicador monto neto en detalle asdss=" + indMontoNetoEnDetalle);
            if (indMontoNetoEnDetalle) {
                total_monto = Math.abs((totalMontoNeto - totalMontoExento) / (1 + (tasaIva / 100)));
                total_IVA = total_monto * (tasaIva / 100);
            } else {
                total_monto = Math.abs((totalMontoBruto - totalMontoExento));
                total_IVA = total_monto * (tasaIva / 100);
            }
            ...
            jaxbTotales.setMntTotal(jaxbTotales.getMntNeto().add(jaxbTotales.getIVA().add(jaxbTotales.getMntExe())));
            // +
            // iva
            // +
            // exento
